...
BOOL DoFileOpenSave(HWND hwnd, BOOL bSave) {
	...
	ofn.hwndOwner = hwnd;
	ofn.lpstrFilter = "Arquivos de Texto (*.txt)\0*.txt\0All Files (*.*)\0*.*\0\0";
	...
	if(bSave) {
		...
		if(GetSaveFileName(&ofn)) {
			if(!SaveFile(GetDlgItem(hwnd, IDC_MAIN_TEXT), szFileName)) {
				MessageBox(hwnd, "Não foi possível salvar o arquivo.", "Erro",MB_OK|MB_ICONEXCLAMATION);
				return FALSE;
			}
		}
	} else {
		...
		if(GetOpenFileName(&ofn)) {
			if(!LoadFile(GetDlgItem(hwnd, IDC_MAIN_TEXT), szFileName)) {
				MessageBox(hwnd, "Não foi possivel carregar o arquivo.", "Erro",MB_OK|MB_ICONEXCLAMATION);
				return FALSE;
			}
		}
	}
		...
		case WM_COMMAND:
			switch(LOWORD(wParam)) {
				...
				case CM_FILE_EXIT:
					...
					break;
				case CM_ABOUT:
					MessageBox (NULL, "SimpleEdit Editor_de_texto_de_windows\nCriado usando Win32 API por Pedro França" , "Sobre...", 0);
					break;
			}
			break;
		case WM_CLOSE:
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpCmdLine, int nCmdShow) {
	...
	hwnd = CreateWindowEx(WS_EX_CLIENTEDGE,"WindowClass","SimpleEdit Editor de Arquivos",WS_OVERLAPPEDWINDOW,
		CW_USEDEFAULT,
		CW_USEDEFAULT,
		320,240,
